...
iNaturalLanguage[expr_] := asciifystring @ basestringify @ FormattingBlock[expr]
...
(* Alternate template boxes for 'prefix' functions *)
 ...
 "PolyGamma2" -> "polygamma", "Algebraics" -> Function["Algebraics"],
 "Booleans" -> Function["Booleans"], "Complexes" -> Function["Complexes"],
 "Integers" -> Function["Integers"], "Primes" -> Function["Primes"],
 "Rationals" -> Function["Rationals"], "Reals" -> Function["Reals"]
 };
...
namedcharacter[c_] := StringReplace[
	c,
	{
		StartOfString~~"Capital"~~r__ :> capitalize[namedcharacter[r]],
		StartOfString~~("DoubleStruck"|"Doubled")~~r__ :> namedcharacter[r],
		StartOfString~~r_~~s___~~EndOfString /; StringFreeQ[s, CharacterRange["A","Z"]] :> ToLowerCase[r]<>s,
		StartOfString~~s__~~EndOfString :> "("<>ToLowerCase[decamel[s]]<>")"
	}
]

capitalize[ s_String ] := StringReplace[s, StartOfString~~a_ :> toUC[a]]
capitalize[ Row[{a_,b___}, s___] ] := Row[{capitalize[a],b}, s]
capitalize[ s_ ] := s

toUC[s_String] := ToUpperCase[s]
toUC["\[Upsilon]"] = "\[Upsilon]"
toUC["\[Nu]"] = "\[Nu]"
