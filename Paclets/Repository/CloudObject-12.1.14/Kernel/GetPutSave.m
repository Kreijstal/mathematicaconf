...
writeObject[obj_CloudObject, content_, mimetype_,
    ...
    Module[{normalizedPermissionRules, result, autoIcons, iconRules, encodedMetaInformation},
        If[permissions =!= Automatic && invalidPermissionsGroups[permissions],
            Return[$Failed]
        ];
        normalizedPermissionRules = serverPermissionsForWriting[obj, permissions, mimetype, head];
        If[normalizedPermissionRules === $Failed, Return[$Failed]];
        ...
        result = Replace[execute[obj, Automatic,
            ...
            Parameters -> Flatten@Join[params, {normalizedPermissionRules,
	...
	Module[{normalizePerms},
		Which[
			...
			permissions === Automatic,
				normalizePerms = Catch[escapeAndNormalizePermissions[$Permissions, mimetype, head], permissionTags];
...
Options[CloudPut] = sortOptions @ objectFunctionOptionsJoin[$objectCreationOptions, {CloudBase -> Automatic,
    IncludeDefinitions -> False}];
Options[iCloudPut] = Join[Options[CloudPut], {"Append" -> False, "MXSerialization" -> Automatic}];

$SystemIs64Bit = SameQ[$SystemWordLength, 64];

Unprotect[$MXSerializationThreshold];
$MXSerializationThreshold = 1048576;
Protect[$MXSerializationThreshold];
...
iCloudPut[expr_, obj:CloudObject[uri_, objopts:OptionsPattern[CloudObject]], mimetype_String, head_Symbol : CloudPut, opts:OptionsPattern[]] :=
    Module[{content, objNew, optsNew, iconRules, metaInformation, permissions, params, defs, contentSize,
        serializeMXOption, useMXSerialization},
        Catch[
            ...
            serializeMXOption = OptionValue["MXSerialization"];
            useMXSerialization = TrueQ[
                    mxVersionIsServerCompatibleQ[] &&
                    (TrueQ[serializeMXOption] || (serializeMXOption === Automatic && contentSize > $MXSerializationThreshold))
                ];

            content = If[useMXSerialization,
                exprToMXBytes[Unevaluated[expr], defs],
                (* Else *)
...
(* Version check for backwards compatibility. Do not convert to ByteArray when evaluating on cloud. *)
If[$VersionNumber >= 12. && !$CloudEvaluation,
...
getCloudVersionNumber[version:None] := 
    With[{con = CloudConnect[]},
        If[con === $CloudUserID,
            If[!StringQ[$CloudVersionNumber], Return[$Failed]]; (* failed to fetch cloud version number *)
...
e : CloudPut[args___] := Null /; (System`Private`Arguments[e, {1, 2}]; False)
...
Options[CloudSave] = sortOptions @ objectFunctionOptionsJoin[$objectCreationOptions, {CloudBase -> Automatic}];
...
e : CloudSave[args___] := Null /; (System`Private`Arguments[e, {1, 2}]; False)
...
Options[CloudGet] = sortOptions @ {CloudBase->Automatic};
...
e : CloudGet[args___] := Null /; (System`Private`Arguments[e, 1]; False)
...
prepareOptionsForWriteObject[msghd_, opts___?OptionQ] :=
    Module[ {optsNew, metaNew},
        ...
        (* CloudPut/CloudExport might inherit Appearance option from CloudDeploy, so we put AppearanceElements information first *)
        metaNew = Join[{handleAppearanceElements[optsNew[AppearanceElements], msghd]}, handleMetaInfo[optsNew[MetaInformation], msghd]];
