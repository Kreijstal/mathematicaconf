...
Options[CopyFile] = sortOptions @ DeleteDuplicates[Join[Options[CopyFile], {"MIMEType" -> Automatic}]];
...
CloudObject /: CopyFile[src_, obj:CloudObject[uri_, opts:OptionsPattern[CloudObject]], o:OptionsPattern[]] :=
    Module[{mimetype, meta, content, parameters, res, sharingList},
    	Catch[
        	...
        	meta = Join[handleMetaInfo[getOptionValue[{opts}, MetaInformation], CopyFile], {handleAppearanceElements[getOptionValue[{opts}, AppearanceElements], CopyFile],
...
$cloudObjectFailures = Alternatives["CloudObjectInvalidPathFailure", "CloudObjectInvalidUUID"]

CloudObject /: CopyFile[failureObj:Failure[$cloudObjectFailures, ___], target:CloudObject[uri_, opts:OptionsPattern[CloudObject]], o:OptionsPattern[]] := failureObj

CopyFile[failureObj:Failure[$cloudObjectFailures, ___], target_, o:OptionsPattern[]] := failureObj

CopyFile[src_, failureObj:Failure[$cloudObjectFailures, ___], o:OptionsPattern[]] := failureObj

SetAttributes[CopyFile, {Protected, ReadProtected}];
...
CloudObject /: RenameFile[src_CloudObject, dest_String] := RenameFile[src, CloudObject[dest]]

RenameFile[src_CloudObject, failureObj:Failure[$cloudObjectFailures, ___]] := failureObj

RenameFile[failureObj:Failure[$cloudObjectFailures, ___], dest:_CloudObject | _String] := failureObj

RenameFile[failureObjSrc:Failure[$cloudObjectFailures, ___], failureObjDest:Failure[$cloudObjectFailures, ___]] := failureObjSrc

SetAttributes[RenameFile, {Protected, ReadProtected}];
...
ParentDirectory[failureObj:Failure[$cloudObjectFailures, ___]] := failureObj

SetAttributes[ParentDirectory, {Protected, ReadProtected}];
...
RenameDirectory[src_CloudObject, failureObj:Failure[$cloudObjectFailures, ___]] := failureObj

RenameDirectory[failureObj:Failure[$cloudObjectFailures, ___], src_CloudObject] := failureObj

RenameDirectory[failureObjSrc:Failure[$cloudObjectFailures, ___], failureObjDest:Failure[$cloudObjectFailures, ___]] := failureObjSrc
...
SetAttributes[RenameDirectory, {Protected, ReadProtected}];
...
DeleteFile[failureObj:Failure[$cloudObjectFailures, ___]] := failureObj

SetAttributes[DeleteFile, {Protected, ReadProtected}];
...
SetAttributes[DeleteDirectory, {Protected, ReadProtected}];
...
CopyDirectory[src_, failureObjDest:Failure[$cloudObjectFailures, ___]] := failureObjDest

CopyDirectory[failureObjSrc:Failure[$cloudObjectFailures, ___], dest_] := failureObjSrc

CopyDirectory[src_CloudObject, failureObjDest:Failure[$cloudObjectFailures, ___]] := failureObjDest

CopyDirectory[failureObjSrc:Failure[$cloudObjectFailures, ___], dest_CloudObject] := failureObjSrc

CopyDirectory[failureObjSrc:Failure[$cloudObjectFailures, ___], failureObjDest:Failure[$cloudObjectFailures, ___]] := failureObjSrc

FileNameJoin[{failureObj:Failure[$cloudObjectFailures, ___], path___}] := failureObj

Protect[CopyDirectory, FileNameJoin]

makeFailureHandling[func_, patt___] := (
    Unprotect[func];
    func[failureObj:Failure[$cloudObjectFailures, ___], patt] := failureObj;
    Protect[func];
)
...
SetAttributes[CopyDirectory, {Protected, ReadProtected}];
    ...
    (* named cloud object *)
        {cloud_, None, userid_, path_, _, _, _} :>
            MatchQ[execute[cloud, "GET", {"files"},
                Parameters -> {"fields" -> "uuid",
                    "path" -> userid <> "/" <> FileNameJoin[path, OperatingSystem -> "Unix"]}],
                {_String, bytes_List /; bytes =!= {}}],
    (* named cloud object inside unnamed directory *)
        {cloud_, diruuid_, userid_, None, _, extraPath_List, _} :>
            MatchQ[execute[cloud, "GET", {"files"},
                Parameters -> {"fields" -> "uuid",
                    "path" -> diruuid<>"/"<> FileNameJoin[extraPath, OperatingSystem -> "Unix"]}],
                {_String, bytes_List /; bytes =!= {}}]
    }
]

SetAttributes[FileExistsQ, {Protected, ReadProtected}];

(* Cloud file name manipulation *)

Unprotect[FileNameJoin]

FileNameJoin[{CloudObject[uri_, rest___], path___}, opts:OptionsPattern[]] := CloudObject[JoinURL[uri, path], rest]
...
CloudObject /: FileFormat[co_CloudObject] :=
    With[{mimeType = cloudObjectInformation[co, FileFormat, "Elements"->"MIMEType"]},
...
CloudObject /: FileHash[obj_CloudObject, rest___] :=
	Module[ {tempfilename, type, res},
		{tempfilename, type} = readObject[obj];
		If[ tempfilename === $Failed,
			Return[$Failed]
		];
		res = FileHash[tempfilename, rest];
		cleanup[tempfilename];
		res /; Head[res] =!= FileHash
	]

SetAttributes[{FileType, DirectoryQ, FileByteCount, FilePrint, FileFormat, FileDate, FileHash}, ReadProtected];
