...
cloudSpecificMIMEToFormatMappings =
    {
        (* notebook mimetypes *)
        "application/vnd.wolfram.notebook" -> "NB",
        "application/vnd.wolfram.nb" -> "NB",
        "application/vnd.wolfram.expression.notebook" -> "NB",
        "application/vnd.wolfram.template" -> "NB",
        "application/vnd.wolfram.notebook.template" -> "NB",
        "application/vnd.wolfram.notebook.element" -> "NB",
        "application/vnd.wolfram.cdf" -> "CDF",
        "application/vnd.wolfram.cloudcdf.html" -> "HTML",
        (* task mimetypes *)
        "application/vnd.wolfram.bundle.document-generator" -> "Bundle",
        "application/vnd.wolfram.bundle.autorefreshed" -> "Bundle",
        "application/vnd.wolfram.bundle" -> "Bundle",
        (* others *)
        "inode/directory" -> "Directory",
        "application/vnd.wolfram.wl" -> "Package",
        "application/vnd.wolfram.cloudexpressionstore" -> "Package",
        "application/vnd.wolfram.httpresponse" -> "Package",
        "application/vnd.wolfram.expression" -> "Package",
        "application/vnd.wolfram.expression." ~~ __ -> "Package"
    }

mimetypeToFormat::usage = "mimetypeToFormat[mimeType, filename] returns the format associated with the mimeType.
If the mapping for the mimeType is not found and the filename is None then Text is returned.
If the filename is not None then FileFormat[filename] is used to determine the format."

formatToMimeType::usage = "formatToMimeType[format] returns the mime-type associated with the format.
If the mapping for the format is not found then application/octet-stream is returned."

If[$VersionNumber > 12.,
    (* For 12.1+ we maintain mimeToFormat mapping to be used only in IconGeneration code (CloudSystem/Icons.m) *)
    mimeToFormat := mimeToFormat = mimeToFormatMapping["ImportExport`FormatToMIMETypeList"];

    cloudSpecificMIMETypeQ[mimeType_] :=
        !MissingQ[
            SelectFirst[Keys@cloudSpecificMIMEToFormatMappings, StringMatchQ[mimeType, #] &]
        ];

    mimetypeToFormat[mimeType_, filename_: None] :=
        Replace[
            mimetypeToFormatLookup[mimeType],
            {
                res_String :> res,
                None :> If[filename =!= None, FileFormat[filename], "Text"]
            }
        ];

    mimetypeToFormatLookup[mimeType_] := mimetypeToFormatLookup[mimeType] =
        Module[{format := (format = Quiet@ImportExport`MIMETypeToFormat[mimeType])},
            Which[
                cloudSpecificMIMETypeQ[mimeType],
                    Last@SelectFirst[cloudSpecificMIMEToFormatMappings, StringMatchQ[mimeType, First[#]] &],
                !FailureQ[format],
                    format,
                True,
                    None
    ...
    formatToMimeType = Composition[
        Replace[s_String :> ToLowerCase[s]],
        Replace @ {
            None | _Missing | Automatic -> "text/plain",
            "wl"  -> "text/plain",
            "css" -> "text/css",
            "string" -> "text/plain", (* string is not mapped to text/plain *)
            fmt_String :> Replace[
                Quiet @ ImportExport`FormatToMIMEType[fmt],
                _?FailureQ :> fmt
            ],
    ...
    ,
    (* Else *)
    mimeToFormat := mimeToFormat = mimeToFormatMapping["ImportExport`GetMIMEType"];
    ...
    mimetypeToFormatLookup[mimeType_] := mimetypeToFormatLookup[mimeType] =
        Replace[
            SelectFirst[mimeToFormat, StringMatchQ[mimeType, First[#]] &],
            {
                Rule[mime_:(String | StringExpression), format_String] :> format,
                _ :> None
            }
        ];
...
mimeToFormatMapping[funcName_String] :=
    Quiet @ DeleteCases[
        Join[
            Flatten @ Map[
            Function[{format}, Function[{mime}, ToLowerCase[mime] -> format] /@ Symbol[funcName][format]],
            $ExportFormats
            ],
            cloudSpecificMIMEToFormatMappings,
            (* On 12.1, ImportExport`MIMETypeToFormat["text/plain;charset=utf-8"] returns Text but on
            12.0 kernel we do not have this mimetype being mapped to a format. If we do not add this
            mapping then mimeToFormatMapping will return $Failed and FileFormat[filename] will get
            called which usually returns the format based on the content of the file (e.g. it will
            return Package for WL specific content). This can effect operations like
            CloudImport[ExportForm[Graphics[Disk[]], "Text"]] *)
...
Options[CloudImport] = sortOptions @ Join[Options[Import], {CloudBase->Automatic}]
...
Options[CloudExport] = sortOptions @ objectFunctionOptionsJoin[$objectCreationOptions, {CloudBase -> Automatic}];
...
exportFormToString[expr_, {"NB", mime_}, rest___] := 
    {$exportFormHandler[expr, "NB", rest], formatToMimeMeta[mime]}

exportFormToString[expr_, {"NBElement", mime_}, rest___] := {
    $exportFormHandler[wrapExportExpr[expr, "NBElement"], "NB", rest], formatToMimeMeta[mime]
}
...
e : CloudExport[args___] := Null /; (System`Private`Arguments[e, {2, 3}]; False)
