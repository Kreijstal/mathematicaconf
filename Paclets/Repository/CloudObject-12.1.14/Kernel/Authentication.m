...
Hold[System`$CloudAccountName];
Hold[System`$RequesterWolframID];
Hold[System`$RequesterCloudUserID];
Hold[System`$RequesterWolframUUID];
Hold[System`$RequesterCloudUserUUID];
Hold[System`CloudUserID];
...
Hold[System`$CloudConnection]
Hold[CloudObject`$FrontEndCloudBase]
Hold[CloudObject`$FrontEndCloudConnection]

Begin["`Private`"]

Unprotect[CloudConnect, CloudDisconnect, $CloudConnected, $UserURLBase, $WolframID, $WolframUUID,
    $CloudUserID, $CloudUserUUID, $RegisteredUserName, $CloudAccountName, $RequesterWolframID,
    $RequesterCloudUserID, $RequesterWolframUUID, $RequesterCloudUserUUID, $CloudVersion,
    $CloudVersionNumber, $CloudWolframEngineVersionNumber, CloudUserID];
...
$checkFECredsQ = True;
frontEndCredsQ[] := TrueQ[And[$Notebooks, CurrentValue["WolframCloudConnected"], $checkFECredsQ]]

getAndProcessFEAuth[base_] := Block[
	{$checkFECredsQ = False}, (*avoid recursive calls from Service Kernel*)
	Catch[
		processFEAuthInfo[getFEAuthInfo[base]],
		$tag
	]
]
getAndProcessFEAuth[___] := getAndProcessFEAuth[extractURLDomain[$CloudBase]]

getFEAuthInfo[] := getFEAuthInfo[$CloudBase] (* base argument is ignored anyway *)
...
$frontendCredUsedQ = False;
processFEAuthInfo[res:{_String}] := Module[{tmp = extractAuthInfo@@res},
	If[extractURLDomain[getValueFromAuthInfo[tmp, "CloudBase"]] === extractURLDomain[$CloudBase],
	    Block[{$frontendCredUsedQ = True}, CloudObject`Internal`SetAuthentication@@res],
	    False
	]
]
...
$connectedCloud = None;
(* $CloudConnected gives 'True' in two cases:
 1) CloudObject`Internal`$ActiveCloudConnection is True
 2) The FE or credentials cache have credentials which can be used for authentication (thus creating
 an active connection)
 Also, $CloudConnected really just means "has the needed tokens to make an authenticated request", 
 not necessarily that we've made a network connection to the cloud in anything like the recent past.
 *)
CloudObject`Internal`CloudConnectStatus[server_] := 
CheckAbort[
    Which[
        (* return True if we're already actively connected *)
        iCloudConnectStatus[server], True,

        (* if the FE has an available connection, use that to become $CloudConnected *)
        frontEndCredsQ[],
        If[TrueQ[getAndProcessFEAuth[server]],
            StringQ[Catch[pingCloudServer[], $tag]],
            False]
        ,
        (* if there's a persisted connection, use that to become $CloudConnected *)
        hasPersistedConnectionQ[server],
        getAndProcessCacheCredentials[server],
        
        (* otherwise, we are decidedly not $CloudConnected *)
        True,
        False
    ],
    AbortProtect[flushCredentials[]; False]
]
CloudObject`Internal`CloudConnectStatus[] := iCloudConnectStatus[]

CloudObject`Internal`$ActiveCloudConnection := iCloudConnectStatus[$CloudBase]

getAndProcessCacheCredentials[server_] := 
Block[{$CloudBase = server, $CacheLogin = True, $PurgeCredFileOnFailure = False},
	(*could probably call 'fetchCredentials' directly, but since creds could be stale still need to ping /files/auth*)
	initiateCloudConnection[];
	StringQ[Catch[callCloudConnectAndPingServer[{}, False, {}, $connectionTimeLimit], $tag]]
]
extractURLDomain[url_String] := 
With[{d = Quiet[URLParse[url, "Domain"]]},
	If[StringQ[d],
...
CloudObject`Internal`RegisteredUserNameStatus[server_String] := Replace[$RUserNameStatus[extractURLDomain@server], _Missing :> None]
...
Options[CloudConnect] = sortOptions @ {CloudBase :> $CloudBase};

$connectionTimeLimit = 60;
...
$cloudconnecthiddenoptions = {
	...
	"Notifications" -> {}, 
	"TimeConstraint" -> $connectionTimeLimit
};
...
initiateCloudConnection[] := 
    If[cloudBaseMatchesFrontEndQ[],
	    connectionLog["Initiating Connection"];
	    callFrontEnd[FrontEnd`WolframCloud`ConnectionInitiated[]]
    ]

establishCloudConnect[server_String] /; cloudBaseMatchesFrontEndQ[] := 
Block[{$CloudBase = server},
	If[SameQ[$CloudUserID, None], (*if user didn't log in*)
		FEConnectFail[1700],
		With[{cloudbase = $CloudBase, username = $CloudUserID, accountName = $CloudAccountName, uuid = $CloudUserUUID},
			connectionLog["Connection Established with ", server];
			callFrontEnd[FrontEnd`WolframCloud`ConnectionEstablished[cloudbase, username, accountName, uuid]]]
	]
]

$verificationURL := URLBuild[{$CloudBase, "files", "auth"}]

$programAndVersion := StringJoin[
...
pingCloudServer[]:= Module[{res,status},
	res=authenticatedURLFetch[
		$verificationURL,
		{"StatusCode", "ContentData"},
		"Headers" -> {"Content-Type" -> "text/plain"},
		"UserAgent" -> $programAndVersion,
		"VerifyPeer" -> False,
...
(* Called by: cloudConnect *)
iConnectAndVerify[args___] := Which[
	(*if there is already an active connection compare the user to $CloudUserID*)
	alreadyConnectedQ[args],
	$CloudUserID,
	(* if FE has creds try loading them and see if they match the login info provided*)
	TrueQ[And[frontEndCredsQ[], frontEndCredsMatchInputQ[args]]],
	getAndProcessFEAuth[$CloudBase];
	pingCloudServer[],
	(*in all other cases signal to FE that we're logging in and call iCloudConnect*)
	True,
	initiateCloudConnection[];
	callCloudConnectAndPingServer[args]
]

(* Called by: iConnectAndVerify *)
callCloudConnectAndPingServer[args___] := With[{res = iCloudConnect[args]},
...
alreadyConnectedQ[creds_, __] := If[
	And[connectedCloudQ[$CloudBase], CloudObject`Internal`CloudConnectStatus[$CloudBase]],
	Switch[creds,
		{}, True,
		_List, SameQ[$CloudUserID, First[creds]],
		_, False
	],
	False,
	False
...
frontEndCredsMatchInputQ[args___] := Catch[
	With[{authInfo = extractAuthInfo@@getFEAuthInfo[$CloudBase]},
	    And[
		  ...
		  SameQ[extractURLDomain[getValueFromAuthInfo[authInfo, "CloudBase"]], extractURLDomain[$CloudBase]],
...
(* Called by: callCloudConnectAndPingServer *)
...
flushCredentials[] := 
    With[{username = $CloudUserID},
        callFrontEnd[FrontEnd`WolframCloud`ConnectionTerminated[username]];
        connectionLog["Flushing Credentials for ", $CloudBase];
		setCloudConnected[False];
		setCloudUserID[None];
		setCloudUserUUID[None];
		setRegisteredUserName[None];
...
(* callFrontEnd[expr] will evaluate expr using MathLink`CallFrontEnd only if this is a kernel with 
   a front end, and its $CloudBase matches the front end's current CloudBase. *)
SetAttributes[callFrontEnd, HoldFirst];
If[TrueQ[$Notebooks],
    callFrontEnd[expr_] := 
        If[cloudBaseMatchesFrontEndQ[],
        	MathLink`CallFrontEnd[expr],
        	Null
        ];
,(* Else it's a standalone kernel, so do nothing quickly *)
    callFrontEnd[expr_] := Null
]

(* cloudBaseMatchesFrontEndQ[]  returns True if there is a front end and if $CloudBase matches that
   of the Front End's service kernel. *)
cloudBaseMatchesFrontEndQ[] := 
    TrueQ[$Notebooks] && currentCloudBaseQ[CloudObject`$FrontEndCloudBase]
...
Options[CloudDisconnect] = sortOptions @ {CloudBase :> $CloudBase}
...
(* fetchCredentials[] retrieves persisted credentials for $CloudBase and on success, 
    sets $CloudUserID etc.
   Returns: Null on success, $Failed on failure to find a stored connection for $CloudBase.
   Called by: iCloudConnect
*)
fetchCredentials[] := Catch[
	If[Not[FreeQ[setConnectionFromStorage[], $Failed]], Throw[$Failed, $tag]];
...
(* Called by: authenticate, immediately above this. *)
...
If[creds === $Canceled || Not[MatchQ[creds, {_String, _String}]],
    ...
    establishCloudConnect[$CloudBase];
    $CloudUserID,
    FEConnectFail[1602]; Message[CloudConnect::notauth]; $Failed]]
]
...
authenticateUsingCredentials[___] := (FEConnectFail[1603]; $Failed)
...
makeAuthURL[Alternatives[
	...
	"www.test.masterkernel.wolframcloud.com",
...
If[Not[$CloudEvaluation],
    Unprotect[$WolframID, $WolframUUID, $CloudUserID, $CloudUserUUID, $RegisteredUserName, $CloudAccountName, $UserURLBase,
        $RequesterWolframID, $RequesterCloudUserID, $RequesterWolframUUID, $RequesterCloudUserUUID,
        $CloudVersion, $CloudVersionNumber, $CloudWolframEngineVersionNumber];
    ...
    $RequesterCloudUserID := $WolframID;
    ...
    $RequesterCloudUserUUID := $WolframUUID;
    $RegisteredUserName := CloudObject`Internal`RegisteredUserNameStatus[$CloudBase];
    $CloudAccountName := CloudObject`Internal`RegisteredUserNameStatus[$CloudBase];
    ...
    Protect[$WolframID, $WolframUUID, $CloudUserID, $CloudUserUUID, $RegisteredUserName, $CloudAccountName,
        $UserURLBase, $RequesterWolframID, $RequesterCloudUserID, $RequesterWolframUUID, $RequesterCloudUserUUID,
        $CloudVersion, $CloudVersionNumber, $CloudWolframEngineVersionNumber];
]
...
setRegisteredUserName[name:(_String | None)] := SetRegisteredUserNameStatus[$CloudBase, name]
...
SetCloudWolframEngineVersionNumber[number:(_Real | None)] := (
	...
	If[number < $VersionNumber,
		Block[{$NumberMarks = False},
			Message[CloudConnect::clver, NumberForm[number, {3, 1}]]
		]
	]
)
...
FEConnectFail[status_Integer] /; UnsameQ[$hasFailed, True] := 
If[cloudBaseMatchesFrontEndQ[],
	Set[$hasFailed, True];
...
makeCredsInfo[version_] := Module[{auth = getAuthentication[version]},
	If[And[auth =!= $Failed, Length[auth] == 5],
		...
		Set[auth["CloudBase"], extractURLDomain[$CloudBase]];
...
getAuthentication[1] := If[ TrueQ[$CloudConnected],
	{$CloudUserID, $CloudUserUUID, $CloudAccountName, getAccessSecret[], getAccessToken[]},
	$Failed
](*legacy version*)
...
$FrontEndCloudBase :=
    CurrentValue[$FrontEndSession, "WolframCloudBaseUrl"]

$FrontEndCloudBase /: Set[$FrontEndCloudBase, cbase_] := 
    Replace[Interpreter["URL"][cbase], {
        validcbase_String :> (CurrentValue[$FrontEndSession, "WolframCloudBaseUrl"] = validcbase)
    }]

frontEndAuthInfo[] :=
    Replace[getFEAuthInfo[], {
        $Failed -> None,
        {compressedInfo_String} :> Uncompress[compressedInfo]
    }]

$FrontEndCloudConnection :=
    Replace[frontEndAuthInfo[], {
    	info_?AssociationQ :> {CloudBase -> info["CloudBase"], CloudUserID -> info["WolframID"]}
    }]

$CloudConnection := 
    If[TrueQ[$CloudConnected],
    	{CloudBase -> $CloudBase, CloudUserID -> $CloudUserID},
    	None
    ]
...
SetAttributes[
    {CloudConnect, CloudDisconnect, $CloudConnected, $UserURLBase, $WolframID, $WolframUUID,
        $CloudUserID, $CloudUserUUID, $RegisteredUserName, $CloudAccountName, $RequesterWolframID,
        $RequesterCloudUserID, $RequesterWolframUUID, $RequesterCloudUserUUID, $CloudVersion,
        $CloudVersionNumber, $CloudWolframEngineVersionNumber, CloudUserID},
