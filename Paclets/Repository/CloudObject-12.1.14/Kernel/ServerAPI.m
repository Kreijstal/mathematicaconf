...
handleErrorDetails["branding-level-not-supported", extra_, head_] := Message[head::appearancenotsup, extra[[1]]]
...
checkError[response_, msghd_Symbol:CloudObject] :=
    If[response === {$Failed, $Failed},
        ...
        With[{res = MatchQ[response, HTTPError[_Integer, ___]]},
            If[res,
                Switch[response,
                    HTTPError[400 | 412, rules:{__Rule} /; KeyExistsQ[rules,"errorCode"] && KeyExistsQ[rules, "extraData"], ___],
                    ...
                    HTTPError[400 | 503, rules:{__Rule} /; KeyExistsQ[rules,"errorCode"] && KeyExistsQ[rules, "errorDetails"], ___],
...
fetchURLAsync[cloud_, url_, elements_, options___] :=
    ( fetchObjURLAsync[cloud, url, Null, options]; {200, {}, {}} )

fetchObjURLAsync[cloud_, url_, callback_, options___] /; useUnauthenticatedRequestQ[cloud] :=
    URLFetchAsynchronous[url, callback, options]

fetchObjURLAsync[cloud_, url_, callback_, options___] := 
    Block[{$CloudBase = cloud},
        authenticatedURLFetchAsynchronous[url, callback, options]
    ]

useUnauthenticatedRequestQ[cloud_] := And[TrueQ[$CloudEvaluation], !sameCloudQ[cloud, $EvaluationCloudBase]]

sameCloudQ[cloud1_, cloud2_] := sameCloudQ[cloud1, cloud2] = SameQ @@ (normalizeCloudServer /@ {cloud1, cloud2})

normalizeCloudServer[cloud_] :=
    normalizeCloudServer[cloud] = StringReplace[URLParse[cloud, "Domain"], StartOfString ~~ "datadrop." -> "www."];
...
getHeader[headers_List, header_String] :=
    Module[{normRes},
        normRes = normalizeHeaders[Rule @@@ headers];
        Lookup[normRes, header, "text/plain"]
    ]

normalizeHeaders[headers:{_Rule...}] := KeyMap[ToLowerCase, Association[headers]]

getUUID[cloud_, path_] := Module[{pathString, result, uuid},
    pathString = JoinURL[path];
    result = Quiet[
        responseToString @ execute[cloud, "GET", {"files"}, Parameters -> {"path" -> pathString, "fields" -> "uuid"}],
        CloudObject::cloudnf (* this error will be handled by handleBadObjectUUID[] later *)
    ];
    uuid = Replace[result, res_String :> Lookup[importFromJSON[result], "uuid", None]];
    log["UUID for path `1`: `2`", pathString, uuid];
    If[MatchQ[uuid, {_String}], First[uuid], None]
]

getCloud[uri_] := First[parseURI[uri]]
    ...
    Module[{parameters, mimetype = OptionValue[Type], body = OptionValue[Body],
        ...
        Replace[
            $lastExecuteResult,
            {
                ...
                HTTPError[status_Integer?Positive, content_] :>
                    HTTPError[status, preprocessContent[content]],
