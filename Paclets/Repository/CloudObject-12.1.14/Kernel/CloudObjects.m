...
System`CloudObjectNameFormat;
System`AppearanceElements;
System`$CloudObjectURLType;
System`CloudObjectURLType;

(*
  Declare the CloudRenderingMethod symbol here.
  It is a Cell option only respected in the cloud,
  but it should be recognized as a System symbol on desktop as well.
  So we rely on the CloudObject paclet as the "delivery mechanism"
  to both worlds.
*)
System`CloudRenderingMethod;
...
(* convert keys of Options[sym] to string before sorting *)
sortOptions = SortBy[Composition[ToString, First]]
...
Options[CloudDebug] = sortOptions @ {DebugLevel -> 1};
        ...
        Switch[protocol,
            "uuid" | "wolfram",
                If[host === None && Length[pathname] >= 1,
                   	{uuid, ext} = checkUUID[First[pathname]];
            ...
            If[MatchQ[pathname, {___, Alternatives[$ViewPattern, filesRoot], __}],
               ...
               (*else URI is of the form .../obj/<username>..." *)
                   ...
                   If[user === "~" || TrueQ[formatNameQ],
                        ...
                        user = Replace[user, $Failed | Except[_String] -> None];
                   ];
...
$objectCreationOptions =
    sortOptions @ {AppearanceElements -> All, CloudObjectNameFormat -> Automatic, CloudObjectURLType -> Automatic, IconRules -> Automatic, MetaInformation -> {}, Permissions -> Automatic};
Options[CloudObject] = sortOptions @ objectFunctionOptionsJoin[$objectCreationOptions, {AutoCopy -> False, SharingList -> {}, SourceLink -> Automatic}];
...
$CloudObjectNameFormatPattern = "UUID" | "UserURLBase" | "CloudUserID" | "CloudUserUUID" | Automatic;

userProtocolQ[user_String] := StringMatchQ[user, "user:"~~__]

processCloudObjectNameFormat[user_?userProtocolQ, format:$CloudObjectNameFormatPattern] :=
    Module[{userNew, res},
        Catch[
...
processCloudObjectNameFormat[user_?userProtocolQ, format_] :=
	(Message[CloudObjectNameFormat::inv, format]; StringDrop[user, StringLength["user:"]])

processCloudObjectNameFormat[user_:"~", format:$CloudObjectNameFormatPattern] :=
    Catch[
        With[{res = userSpecificationFromNameFormat[user, format]},
            If[res === None, 
                Message[CloudObjectNameFormat::una, format];
                $UserURLBase,
                res
            ]
        ],
...
cloudObjectFailureObject["invuuid", uuid_String] :=
    (Message[CloudObject::invuuid, uuid]; Throw[Failure["CloudObjectInvalidUUID", 
        <|"MessageTemplate" -> CloudObject::invuuid, "MessageParameters" -> {uuid}|>], InvUUID])
...
(
    If[$CloudUserID === None,
        CloudConnect[];
        If[!TrueQ[$CloudConnected], Message[CloudObject::notauth]; Throw[$Failed, $tag]]
    ];
    Switch[format,
...
(* This will avoid calling /users api for an authenticated user *)
nameFormatOptionToValue[usr_, format_] /; SameQ[usr, $UserURLBase] :=
    nameFormatOptionToValue["~", format]
...
nameFormatOptionToValue[usr_, format_] :=
    Module[{json, importJSON, userInfo, res},
    	If[StringMatchQ[usr, userUUIDPrefix ~~ $uuidPattern] && format === "CloudUserUUID",
        	...
        	If[json === $Failed,
            	None,
            	importJSON = handleUsersJSONObject[importFromJSON[json]];
...
$uuidPattern = 
    ...
    "4" ~~ Repeated[HexadecimalCharacter, {3}] ~~ "-" ~~ 
    Repeated[HexadecimalCharacter, {4}] ~~ "-" ~~ 
...
CloudObject["UUID" -> uuid_, opts:OptionsPattern[]] := CloudObject["uuid:" <> uuid, opts]

CloudObject[___, "UUID" -> uuid_, ___, opts:OptionsPattern[]] :=
    (Message[CloudObject::invarg, uuid]; Failure["CloudObjectInvalidArgument", <|"MessageTemplate" -> CloudObject::invarg|>])
...
CloudObject[uuidPrefixString_String ? (StringStartsQ[#, "uuid:"] &), opts:OptionsPattern[]] :=
    With[{uuid = StringDrop[uuidPrefixString, 5], base = getCloudBase[$CloudBase], urlType = OptionValue[CloudObjectURLType]},
        If[UUIDQ[uuid],
            Catch[
                CloudObject[JoinURL[{base, handleCloudObjectURLType[urlType, CloudObject], uuid}], opts],
                BadOption
            ],
            Message[CloudObject::invuuid, uuid];
            Failure["CloudObjectInvalidUUID", <|"MessageTemplate" -> CloudObject::invuuid, "MessageParameters" -> {uuid}|>]
        ]
    ]
...
e : CloudObject[args___] := Null /; (System`Private`Arguments[e, {0, 2}]; False)

Options[cloudObject] = Options[CloudObject];	

(* optimize the very common case of base/obj/{uuid} *)
        ...
        If[ illegalObjectPathQ[uri],
            ...
            If[ nameFormat === "UUID",
                ...
                If[ !(StringQ[cloud] && UUIDQ[uuid]),
                    $Failed,
                    newURI = JoinURL[{cloud, handleCloudObjectURLType[urlType, CloudObject], uuid}];
...
cloudObject[uri_String, opts:OptionsPattern[]] :=	
	Module[{nameFormat, urlType, fullURLQ, newURI, cloud, uuid, user, path, ext, extraPath, search, res, finalURL},
        ...
        If[FailureQ[res],
            res,
            res = If[!fullURLQ && (nameFormat === "UUID"),
                finalURL = reconstructUuidURI[res];
                If[finalURL === $Failed,
                Message[CloudObjectNameFormat::una, "UUID"];
                    res
                    ,
                    finalURL
                ],
            ...
            If[res === $Failed,
                $Failed,
                (* res is a full named url, thus 3rd argument of finalizeURL can be set to True at
                this point to avoid a server call *)
                CloudObject[finalizeURL[res, urlType, True], optFinal[{opts}]]]
        ]
    ]
...
reconstructUuidURI[uri_String] :=
	Module[{cloud, uuid},
		{cloud, uuid} = getCloudAndUUID[uri]; 
		If[!(StringQ[cloud] && UUIDQ[uuid]), Return[handleBadObjectUUID[uuid, CloudObject[uri]]]];
		JoinURL[{cloud, $CloudObjectsRoot, uuid}]
	]
...
(* In desktop Mathematica, use MakeBoxes rather than Format *)
    ...
    Unprotect[CloudObjectInformationData];
    ...
    SetAttributes[CloudObjectInformationData, {Protected, ReadProtected}];
]
...
SetAttributes[{$CloudObjectNameFormat, $CloudObjectURLType}, ReadProtected];
SetAttributes[CloudObject, {Protected, ReadProtected}];
(* Protect Options *)
Protect[AppearanceElements, CloudObjectNameFormat, CloudObjectURLType, IconRules, MetaInformation, Permissions, AutoCopy, SharingList, SourceLink];
