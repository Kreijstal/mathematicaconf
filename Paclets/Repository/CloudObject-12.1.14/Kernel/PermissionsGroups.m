...
Unprotect[PermissionsGroup, CreatePermissionsGroup, PermissionsGroups, AddUsers, RemoveUsers, SetUsers,
...
PermissionsGroup[url_]["Creator"] :=
    CloudObjectInformation[CloudObject[url], "OwnerCloudUserID"]
...
e : CreatePermissionsGroup[args___] := Null /; (System`Private`Arguments[e, {1, 2}]; False)
...
e : AddUsers[args___] := Null /; (System`Private`Arguments[e, 2]; False)

SetUsers[group_, users_] := iAddSetUsers["PUT", SetUsers, group, users]
  
e : SetUsers[args___] := Null /; (System`Private`Arguments[e, 2]; False)

(* RemoveUsers *)

RemoveUsers[group_PermissionsGroup, user_String] := 
    With[{userVal = If[UUIDQ[user], userUUIDPrefix <> user, user]},
        Replace[execute[group, "DELETE", {"group", userVal}],
            {
            HTTPError[404, ___] :> (Message[RemoveUsers::invusr, user]; $Failed),
            other_ :> responseCheck[other, RemoveUsers]
            }
        ]
    ]
...
RemoveUsers[group_String, rest__] := RemoveUsers[PermissionsGroup[group], rest]
RemoveUsers[groups_List, rest__] := RemoveUsers[#, rest] & /@ groups

RemoveUsers[___] := (Message[RemoveUsers::notparam]; $Failed)
...
PermissionsGroupMemberQ[group:PermissionsGroup[url_], user_String] := 
	Module[{members, json, data, userData},
        ...
        (* else *)
        	...
        	If[StringQ[json],
            data = handleUsersJSONObject[importFromJSON[json]];
...
SetAttributes[
    {PermissionsGroup, CreatePermissionsGroup, PermissionsGroups, AddUsers, RemoveUsers, SetUsers,
        PermissionsGroupMemberQ},
    {Protected, ReadProtected}
];
SetAttributes[$PermissionsGroupBase, ReadProtected];
