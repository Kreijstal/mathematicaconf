...
e : CloudShare[args___] := Null /; (System`Private`Arguments[e, {1, 2}]; False)

CloudShare[failureObj:Failure[$cloudObjectFailures, ___], arg_] := failureObj

SetAttributes[CloudShare, {Protected, ReadProtected}];

Unprotect[CloudUnshare];
...
CloudUnshare[obj_CloudObject] :=
    Module[ {cloud, uuid, delRes},
        {cloud, uuid} = Quiet[getCloudAndUUID[obj]];
        If[ StringQ[cloud] && UUIDQ[uuid],
            delRes = deleteAllSharees[cloud, uuid, CloudUnshare];
            If[delRes === $Failed, Return[$Failed]];
...
CloudUnshare[obj_CloudObject, sharees:{shareesSpecs..}] :=
    Module[ {cloud, uuid, persOld, persNew, delRes},
    	{cloud, uuid} = Quiet[getCloudAndUUID[obj]];
        If[ StringQ[cloud] && UUIDQ[uuid],
            delRes = deleteSubsetSharees[cloud, uuid, sharees, CloudUnshare];
            If[delRes === $Failed, Return[$Failed]];
...
e : CloudUnshare[args___] := Null /; (System`Private`Arguments[e, {0, 2}]; False)

CloudUnshare[failureObj_Failure, arg___] := failureObj

SetAttributes[CloudUnshare, {Protected, ReadProtected}];
...
(* This clears previous sharees and replace it with new *) 
