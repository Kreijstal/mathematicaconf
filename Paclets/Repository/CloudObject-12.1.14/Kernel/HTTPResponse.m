...
Unprotect[HTTPResponse, HTTPRequest, HTTPErrorResponse]
...
$httpRequestPattern = With[{p = $urlPattern}, HoldPattern[HTTPRequest[p, _?AssociationQ, ___]]]
$httpResponsePattern = With[{p = $BodyPattern}, HoldPattern[HTTPResponse[p, _?AssociationQ, ___]]]
$httpPattern  = $httpRequestPattern|$httpResponsePattern
...
Options[HTTPResponse] = sortOptions @ {
...
$httpRules := $httpRules = Dispatch @ Apply[
    ...
    {$httpResponsePattern, "Properties"} :> {
        "Body",
        "BodyByteArray",
        "BodyBytes",
        "CharacterEncoding",
    ...
    {res:$httpResponsePattern, "StatusCodeDescription"} :> 
    ...
    {res:$httpRequestPattern, "FormRules"} :> 
        Replace[
            Replace[
    ...
    {res:$httpRequestPattern, "MultipartElements"|"MultipartContent"} :> 
        Replace[
            HTTPProperty[res, "FormRules"], {
                _[key_, File[s_, ___]] :> key -> <|
                    "InMemory" ->  False, 
                    "FileName" ->  s, 
                    "OriginalFileName" -> FileNameTake[s]
                |>,
                _[key_, s_] :> key -> <|
                    "InMemory" -> True,
                    "ContentString" -> s
...
Options[HTTPRequest] := sortOptions @ With[{opts = Options[URLRead]}, Unprotect[HTTPRequest]; Options[HTTPRequest] = opts; Protect[HTTPRequest]; opts]
...
SetAttributes[{HTTPResponse, HTTPRequest, HTTPErrorResponse}, {Protected, ReadProtected}];
