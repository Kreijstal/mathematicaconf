...
optionsCloudObject /: Verbatim[Options][obj_optionsCloudObject] :=
    Options[obj, cloudObjOptionsKeys];

cloudObjOptionsKeys = Sort @ Join[Keys @ Options[CloudObject], {"Active"}];
...
Failure /: Verbatim[Options][failureObj:Failure[$cloudObjectFailures, ___], args___] := failureObj

Protect[Failure]

Unprotect[SetOptions]

SetOptions[failureObj:Failure[$cloudObjectFailures, ___], o:OptionsPattern[]] := failureObj
...
setOption[Permissions -> permissions_][obj_CloudObject] :=
    Module[{cloud, uuid, mimetype, accessJSON},
		...
		accessJSON = Catch[escapeAndNormalizePermissions[permissions, mimetype, SetOptions], permissionTags];
...
lookupOption[Permissions][obj_CloudObject] := 
    Module[{cloud, uuid, permjson},
        ...
        fromServerPermissions[permjson]
    ]
...
(*****************************************************************************)
(* AppearanceElements *)
setOption[(Rule | RuleDelayed)[AppearanceElements, appElements:"Branding" | {"Branding"} | {} | None | All]][obj_CloudObject] :=
    SetOptions[obj, MetaInformation -> handleAppearanceElements[appElements, SetOptions]]

setOption[AppearanceElements -> appElements_][obj_CloudObject] :=
(
    Message[SetOptions::invappelem, appElements];
    $Failed
)
    
lookupOption[AppearanceElements][obj_CloudObject] := 
    Module[{meta},
    	meta = Options[obj, MetaInformation -> "__AppearanceElements"];
    	If[meta === $Failed,
    		$Failed,
    		Lookup[Lookup[meta, MetaInformation], "__AppearanceElements", All]
    	]
    ]      
...
(*****************************************************************************)
(* common section to define hidden options and related functions *)
$hiddenOptions = {
    (* MXSerialization -> True will serialize content to MX
       MXSerialization -> False will not serialize content to MX
       MXSerialization -> Automatic will serialize content to MX only if content size exceeds threshold *)
    "MXSerialization" -> Automatic
    };

(* For a given symbol, since hidden options are not supposed to be accessible using Options call, the final rule
of OptionValue[sym, opts, name] needs to be modified to consider hidden options. *)
Unprotect[OptionValue];

OptionValue[sym: CloudDeploy | CloudPublish | CloudPut | cloudPut, opts_List, optionName_] := 
    OptionValue[{sym, $hiddenOptions}, opts, optionName]
