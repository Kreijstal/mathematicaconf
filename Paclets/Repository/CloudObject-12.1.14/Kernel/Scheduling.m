...
Options[ScheduledTask] = Options[ContinuousTask] = sortOptions @ {
    NotificationFunction -> Automatic,
    TimeZone -> Automatic,
    AutoRemove -> False
};

ScheduledTask /: CloudDeploy[task_ScheduledTask, co_CloudObject, opts:OptionsPattern[]] :=
    Catch[iCloudDeployScheduledTask[task, co, opts], $tag]

SetAttributes[ContinuousTask, {HoldAll, ReadProtected}];
Options[ContinuousTask] = sortOptions @ {
    NotificationFunction -> Automatic,
...
iCloudDeployScheduledTask[st:ScheduledTask[expr_, sched_, o:OptionsPattern[]], obj:CloudObject[uri_String, ___], deploymentOptions:OptionsPattern[]] :=
Module[{cloud, uuid, name, continuous = TrueQ[Lookup[Flatten[{o}], "Continuous", False]],
    runImmediately = False, cronned, params, opts = Flatten[{o}], endpoint, taskJson, rJson, deployOpts},
    ...
    deployOpts = Join[Flatten[{deploymentOptions}], Options[CloudDeploy]];

    taskJson = generateTaskJson[st, {name, uuid}, cronned, Join[opts, {"RunImmediately" -> runImmediately}], deployOpts];
    If[taskJson === $Failed,
        Throw[$Failed, $tag]
    ];
...
exprDefinitionsToString[expr_, True] :=
...
exprDefinitionsToString[expr_, False] := ""
...
generateTaskJson[st:ScheduledTask[expr_, __], {name:(_String|Null), uuid:(_String|Null)}, {start_, stdSched_, end_}, 
    o:{___?OptionQ}, deploymentOptions:{___?OptionQ}] :=
Module[{opts, cont, type = "co", strexpr, result, startTimestamp, endTimestamp},
    ...
    If[!BooleanQ[Lookup[deploymentOptions, IncludeDefinitions]],
		Message[General::invincludedf, Lookup[deploymentOptions, IncludeDefinitions]];
		Return[$Failed];
	];

    strexpr = exportToJSON[{Replace[exprDefinitionsToString[Unevaluated[expr], Quiet[Lookup[deploymentOptions, IncludeDefinitions, True]]], "\n\n" -> ""],
        ...
        Sequence @@ Map[ToString[#, InputForm]&, DeleteCases[FilterRules[opts, Options[ScheduledTask]], Rule["RunImmediately", _]]]}, "Compact" -> True];
...
generateTaskJson[ar:AutoRefreshed[expr_, _, fmt_, ___], {name:(_String|Null), uuid:(_String|Null)}, {start_, stdSched_, end_},
  o:{___?OptionQ}, deploymentOptions:{___?OptionQ}] :=
    Module[{opts, cont, type, strexpr, result,startTimestamp, endTimestamp},
      ...
      {type, strexpr} = {"ar", StringTrim[exportToJSON[{exprDefinitionsToString[Unevaluated[expr], Quiet[Lookup[deploymentOptions, IncludeDefinitions, True]]], "AutoRefreshed",
        neutralContextBlock[ToString[Unevaluated[expr], InputForm, CharacterEncoding -> "PrintableASCII"]],
...
If[$VersionNumber < 12.1,
	...
	CloudObject`CloudTasks[] := If[TrueQ[$CloudConnected],
		Block[{cloudTasks = Catch[iCloudTasks[], $tag]},
		    If[ListQ[cloudTasks],
		    	cloudTasks,
		    	{}
			]
	...
	ScheduledTasks[] := Join[Tasks[], CloudObject`CloudTasks[]]
]

iCloudTasks[] := Module[
    {raw, json, tasks, msghd = ScheduledTasks},
...
computeTaskValue["EndDate", data_] := Rule["EndDate",
...
autoRefreshedStaging[AutoRefreshed[expr_, sched_, fmt_, o:OptionsPattern[]], co_CloudObject, oCD:OptionsPattern[]] :=
    Module[{cloud, uuid, name, runImmediately = False, cronned, continuous = TrueQ[Lookup[Flatten[{o}], "Continuous", False]],
        ...
        params = {"bundle" -> ($lastJSON = generateTaskJson[taskExpr, {name, uuid}, cronned, Join[Options[ScheduledTask], Flatten[{o}],
                  {"Visible" -> False, "RunImmediately" -> runImmediately}], Join[Flatten[{oCD}], Options[CloudDeploy]]])};
...
Protect[NotificationFunction, IncludeGeneratorTasks, RestartInterval];(* Protect Options *)
SetAttributes[
    {ScheduledTask, CloudObject, ScheduledTasks, Tasks, EvaluateScheduledTask, ScheduledTaskInformation,
        ScheduledTaskInformationData, ContinuousTask, AbortScheduledTask, AutoRefreshed, Tasks`TaskEvaluate,
        CloudObject`CloudTasks,
        TaskExecute, TaskAbort, NextScheduledTaskTime, ScheduledTaskActiveQ}
    ,
