...
(* CloudObjects *)
Unprotect[CloudObjects];

Options[CloudObjects] = sortOptions @ {"Directory" -> Automatic, "Type" -> All, CloudBase -> Automatic, CloudObjectNameFormat -> Automatic}
...
queryTypeValue["Notebook"|Notebook] := StringRiffle[notebookMIMETypes, ","];
...
queryTypeValue[HoldPattern[symbol: APIFunction | FormFunction | Delayed | ScheduledTask]] := expressionMimeType[symbol];
...
notebookMIMETypes = {"application/mathematica",
    "application/vnd.wolfram.expression.notebook",
    "application/vnd.wolfram.mathematica",
    "application/vnd.wolfram.nb",
    "application/vnd.wolfram.notebook",
    "application/vnd.wolfram.notebook.element",
    "application/vnd.wolfram.template"};
...
SetAttributes[CloudObjects, {Protected, ReadProtected}];
...
Unprotect[CloudObjectInformation];
...
SetAttributes[CloudObjectInformation, {Protected, ReadProtected}];
...
(***********************************************************************)
(* Implementation of private utility functions that calls /files api and perform specific operations 
    like getting user's favorite files, recent files, shared files etc. *)

Options[getFavoriteFilesInfo] = {"Elements" -> Automatic}
(* Returns information of the user's favorite files. User here is the one who calls this function and is cloud connected. *)
getFavoriteFilesInfo[opts:OptionsPattern[]] := getFilesInfo["favorite", opts]

Options[getRecentFilesInfo] = {"Elements" -> Automatic}
(* Returns information of the user's recent files *)
getRecentFilesInfo[opts:OptionsPattern[]] := getFilesInfo["recent", opts]

Options[getSharedByYouFilesInfo] = {"Elements" -> Automatic}
(* Returns information of all the files shared by the user *)
getSharedByYouFilesInfo[opts:OptionsPattern[]] := getFilesInfo["sharedByYou", opts]

Options[getSharedWithYouFilesInfo] = {"Elements" -> Automatic}
(* Returns information of all the files shared with the user *)
getSharedWithYouFilesInfo[opts:OptionsPattern[]] := getFilesInfo["sharedWithYou", opts]

Options[getUnnamedFilesInfo] = {"Elements" -> Automatic}
(* Returns information of all the unnamed files of the user *)
getUnnamedFilesInfo[opts:OptionsPattern[]] := getFilesInfo["path" -> "", opts]

Options[getFilesInfo] = {"Elements" -> Automatic}

getFilesInfo[requestedParam_String, opts:OptionsPattern[]] := getFilesInfo[requestedParam -> True, opts]

getFilesInfo[query:Rule[param_, value_], opts:OptionsPattern[]] := 
    Module[{fields, elements, json, info},
        If[!queryParameterSupportedQ[param], Return[$Failed]];
        elements = OptionValue["Elements"];
        fields = resolveInfoFields[elements];
        If[fields === {$Failed}, Return[$Failed]];

        json = Replace[
            execute[$CloudBase, "GET", {"files"}, Parameters -> {param -> value, "fields" -> commaSeparated[fields]}],
            {
                {_String, content_List} :> FromCharacterCode[content],
                HTTPError[404, ___] :> (
                    Message[CloudObject::cloudnf]; 
                    Return[$Failed]
                    ), 
                other_ :> (
...
queryParameterSupportedQ[param_] := MemberQ[$supportedFilesInfoQueryParameters, param]
$supportedFilesInfoQueryParameters = {"favorite", "recent", "sharedByYou", "sharedWithYou", "path"};
    ...
    Module[{elements = OptionValue["Elements"], mimetype = Lookup[info, "mimeType", None], 
        ...
        Do[
            ...
            <|
                ...
                "OwnerCloudUserUUID" -> info["ownerUUID"],
                "OwnerWolframID" :> Lookup[info["owner"], "email", Missing["Unavailable"]],
                "OwnerCloudUserID" :> Lookup[info["owner"], "email", Missing["Unavailable"]],
                "MIMEType" -> mimetype,
                "MimeType" -> mimetype,
                "FileType" ->
...
$jsonFields = <|
    ...
    "OwnerCloudUserUUID" -> "ownerUUID",
    "OwnerWolframID" -> "owner",
    "OwnerCloudUserID" -> "owner",
    "MIMEType" -> "mimeType",
    "MimeType" -> "mimeType",
    "FileType" -> "mimeType",
