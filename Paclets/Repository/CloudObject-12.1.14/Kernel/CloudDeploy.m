...
Options[CloudDeploy] = sortOptions @ objectFunctionOptionsJoin[$objectCreationOptions, {CloudBase -> Automatic,
    IncludeDefinitions -> True, SourceLink -> Automatic, AutoCopy -> False}];
...
e : CloudDeploy[args___] := Null /; (System`Private`Arguments[e, {1, 2}]; False)
...
handleCloudDeployOptions[obj_CloudObject, useOriginal_?BooleanQ, msghd_, opts___?OptionQ] :=
    Module[ {optsNew, metaNew, includeDefNew},
        ...
        metaNew = Join[handleMetaInfo[optsNew[MetaInformation], msghd], {
            handleAppearanceElements[optsNew[AppearanceElements], msghd], 
        ...
        Join[FilterRules[Normal[optsNew], {CloudObjectNameFormat, CloudObjectURLType, IconRules, $hiddenOptions}],
...
handleAppearanceElements[None, msghd_] := handleAppearanceElements[{}, msghd]
handleAppearanceElements[appElems_String, msghd_] := handleAppearanceElements[{appElems}, msghd]
handleAppearanceElements[appElems:{"Branding"}| All | {}, msghd_] := "__AppearanceElements" -> appElems
handleAppearanceElements[appElems_, msghd_] := (Message[msghd::invappelem, appElems]; Throw[$Failed, BadOption])
...
permissionsFormatValidation[{}, msghd_] := "Private"
...
handleOptsForURL[symbol_Symbol, opts_List] :=
	Module[{nameFormat, urlType},
		nameFormat = Quiet[OptionValue[symbol ,opts, CloudObjectNameFormat], OptionValue::nodef];
		urlType = Quiet[OptionValue[symbol, opts, CloudObjectURLType], OptionValue::nodef];
...
Unprotect[CloudPublish];

Options[CloudPublish] = sortOptions @ Normal[Association[Options[CloudDeploy], Permissions -> {All -> Automatic}, AutoCopy -> True]];
    ...
    Module[{res, optsNew},
        Catch [
            ...
            If[MatchQ[res, _CloudObject],
            	...
            	optsNew = FilterRules[handleCloudDeployOptions[obj, True, CloudPublish, opts],
            	    Except[IncludeDefinitions | CloudObjectNameFormat | CloudObjectURLType]];
...
e : CloudPublish[args___] := Null /; (System`Private`Arguments[e, {0, 2}]; False)

SetAttributes[CloudPublish, {Protected, ReadProtected}];

SetAttributes[{$SourceLink, $CloudEvaluation}, {ReadProtected}];
(* Protect Options *)
Protect[CloudBase, SourceLink];
