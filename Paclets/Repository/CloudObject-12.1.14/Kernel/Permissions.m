...
normalizePermissions[{}, type_, head_] := 
...
permissionTags = UnsupportedConstraintPatternTag | InvalidConstraintPatternValueTag | InvalidAllowingTag;
...
fromServerPermissions[permjson_] := Replace[
    importFromJSON[permjson],
    {
        serverPermissions_List :>
            DeleteCases[Map[convertFromServerPermissions, serverPermissions], Rule[x_, {}]],
...
modifyPermissions[obj_CloudObject, pers:{Rule[_, _] ..}, head_] :=
    Module[{persList, persExisting, persCombined, persNew, modifiedPers, notAllValid},
        Catch[
            ...
            If[persList == {}, (Message[head::invperm, pers]; Return[$Failed])];
...
Unprotect[SetPermissions, ClearPermissions];

(* SetPermissions *)    
SetPermissions[pers_] :=
    SetPermissions[$EvaluationCloudObject, pers] 

SetPermissions[obj_, {}] := SetPermissions[obj, "Private"]
...
e : SetPermissions[args___] := Null /; (System`Private`Arguments[e, {1, 2}]; False)
...
e : ClearPermissions[args___] := Null /; (System`Private`Arguments[e, {1, 2}]; False)

ClearPermissions[failureObj:Failure[$cloudObjectFailures, ___], class_] := failureObj

SetAttributes[{SetPermissions, ClearPermissions, ApplicationIdentificationKey}, {Protected, ReadProtected}];
SetAttributes[{$Permissions}, ReadProtected];
