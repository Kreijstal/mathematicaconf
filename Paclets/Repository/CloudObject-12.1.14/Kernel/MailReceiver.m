...
Unprotect[MailReceiverFunction, MailResponseFunction, ReturnReceiptFunction];
...
createShortID[co_]:=Block[{url, id,shortid},
    ...
    If[$UserCloudObjectURLs,
        ...
        url = URLBuild[{$CloudBase, $objRoot, id}]
    ];
...
Options[System`MailReceiverFunction] = CloudObject`Private`sortOptions @
    {System`MailResponseFunction->Automatic, System`ReturnReceiptFunction->Automatic}
...
getMailReceiverValue[key: ("AttachmentDecodedData"|"AttachmentDetails"|"AttachmentList"), as_, rawmail_, _]:=
	Replace[
		Quiet[ImportString[rawmail,{"MBOX",key,1}]],
	_?FailureQ->{},{0}]
...
getMailReceiverValue["OriginatingTimezone"|"OriginatingTimeZone",as_, _, _]:=takeTimeZone[checkHeaders[as, "Date"]]
getMailReceiverValue["ServerOriginatingDate",as_, _, _]:=takelast[Flatten[{checkHeaders[as, "Received"]}],"DateTime"]
getMailReceiverValue["ServerOriginatingTimezone"|"ServerOriginatingTimeZone",as_, _, _]:=takelast[Flatten[{checkHeaders[as, "Received"]}],"TimeZone"]
...
SetAttributes[{MailReceiverFunction, MailResponseFunction, ReturnReceiptFunction}, {Protected, ReadProtected}];
