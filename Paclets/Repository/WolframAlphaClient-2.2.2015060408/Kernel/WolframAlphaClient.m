...
DynamicModule[{emphasizeErrors = False},
	...
	Which[
		...
		Framed[#,
			Background -> RGBColor[1., 0.980392, 0.921569], (* FIXME: this should come from the stylesheet *)
			BaselinePosition -> Baseline,
			FrameMargins -> 0,
			FrameStyle -> RGBColor[1., 0.504768, 0.], (* FIXME: this should come from the stylesheet *)
			RoundingRadius -> 4
		],
		$CloudControlEqual,
		Panel[
...
LinguisticAssistantMakeExpression[query_String, boxes:"None", querystate: {___, "Online" -> False, ___}, fmt_] :=
	With[{assoc = Association @ Flatten @ {"MessageTemplate" -> cachedFrontEndResource["WAStrings", "MessageTemplate:Offline"], "Query" -> query, querystate}}, HoldComplete @ Failure["WolframAlphaOffline", assoc]]

LinguisticAssistantMakeExpression[query_String, boxes:"None", querystate: {___, "Allowed" -> False, ___}, fmt_] :=
	With[{assoc = Association @ Flatten @ {"MessageTemplate" -> cachedFrontEndResource["WAStrings", "MessageTemplate:Disallowed"], "Query" -> query, querystate}}, HoldComplete @ Failure["WolframAlphaDisallowed", assoc]]

LinguisticAssistantMakeExpression[query_String, boxes:"None", querystate: {___, "mparse.jsp" -> _, "$TimedOut" -> _, ___}, fmt_] :=
	With[{assoc = Association @ Flatten @ {"MessageTemplate" -> cachedFrontEndResource["WAStrings", "MessageTemplate:QueryTimedOut"], "Query" -> query, querystate}}, HoldComplete @ Failure["WolframAlphaQueryTimedOut", assoc]]

LinguisticAssistantMakeExpression[query_String, boxes:"None", querystate: {___, "$TimedOut" -> _, ___}, fmt_] :=
	With[{assoc = Association @ Flatten @ {"MessageTemplate" -> cachedFrontEndResource["WAStrings", "MessageTemplate:MParseTimedOut"], "Query" -> query, querystate}}, HoldComplete @ Failure["WolframAlphaParseTimedOut", assoc]]

LinguisticAssistantMakeExpression[query_String, boxes:"None", querystate: {___, "mparse.jsp" -> _, "query.jsp" -> _, ___}, fmt_] :=
	With[{assoc = Association @ Flatten @ {"MessageTemplate" -> cachedFrontEndResource["WAStrings", "MessageTemplate:NoParse"], "Query" -> query, querystate}}, HoldComplete @ Failure["NoWolframAlphaParse", assoc]]
...
doNewFastParse[query_, contextinfo: $cscePat, Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_], Dynamic[querystate_]] :=
	Block[{rawparse, online, allowed, now},
		...
		online = If[query === "error: offline", False, CurrentValue["InternetConnectionAvailable"]];
		allowed = If[query === "error: disallowed", False, CurrentValue["AllowDownloads"]];
